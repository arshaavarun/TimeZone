...
- `@app.before_request` owner-login guard (first-run setup → login; see **Owner auth**), registered before the archived-client guard so it runs first,
...
**Owner auth.** Single-owner app: one shared password, **hashed** in `app_settings.owner_password_hash` (werkzeug). The `_require_login` guard forces `/setup` until a password exists, then `/login` for every page; exempt endpoints (login/setup/logout/static) are in `_AUTH_EXEMPT` (`__init__.py`). Sessions are long-lived ("stay logged in", `SESSION_DAYS`). The Flask `secret_key` is generated once into a **gitignored `.flask_secret`** file, never hard-coded. CSRF is deliberately deferred. The test harness seeds a password and marks the shared client authenticated so the suite runs past the guard; the `auth` test group exercises the gate itself. Helpers: `services.owner_password_is_set / set_owner_password / check_owner_password`; routes in `views_auth.py`.
...
- **Keyboard accessibility** — keep the app keyboard-operable. Convention: **Tab/Shift+Tab** moves *between* controls; **arrow keys** move only *within* a widget — tabs (ARIA `tablist` + roving `tabindex` + Arrow/Home/End, wired in `app.js`), the calendar grid (Left/Right a day, Up/Down a week), the custom dropdown menus (via `setupMenuKeys`), and the combobox lists; native `<select>`s get arrows for free. Any new interactive control must: be focusable with a visible **`:focus-visible`** ring, have an accessible name (visible text, `title`, or `aria-label` — icon SVGs are `aria-hidden`), and expose state where relevant (`aria-expanded` on disclosures, `aria-selected` on tabs). Custom (non-`<button>`) toggles use `role="button"` + Enter/Space. Flash toasts live in an `aria-live` region.
